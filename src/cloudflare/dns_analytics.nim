...
proc getZonesZoneIdDnsAnalyticsReport*(client: CloudflareClient,
                                       zoneId: types.DnsAnalyticsIdentifier,
  ...
  q["sort"] = $sort
  q["filters"] = $filters
  let res = await client.httpGET(fmt"/zones/{zoneId}/dns_analytics/report", q)
...
proc getZonesZoneIdDnsAnalyticsReportBytime*(client: CloudflareClient,
                                             zoneId: types.DnsAnalyticsIdentifier,
                                             metrics: types.DnsAnalyticsMetrics = default(types.DnsAnalyticsMetrics),
                                             dimensions: types.DnsAnalyticsDimensions = default(types.DnsAnalyticsDimensions),
                                             since: types.DnsAnalyticsSince = default(types.DnsAnalyticsSince),
  ...
  let res = await client.httpGET(fmt"/zones/{zoneId}/dns_analytics/report/bytime", q)
  let body = await res.body
  case res.code
  of Http200:
    result = fromJson(body, JsonNode)
