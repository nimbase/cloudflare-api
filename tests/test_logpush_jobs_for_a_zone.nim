...
suite "logpush_jobs_for_a_zone serialization":
...
suite "logpush_jobs_for_a_zone endpoints":
  test "GET /zones/{zone_id}/logpush/jobs":
    let client = initCloudflareClient("test-key")
    client.baseUri = "http://127.0.0.1:" & $int(startMock())
    discard waitFor client.getZonesZoneIdLogpushJobs("test")

  test "GET /zones/{zone_id}/logpush/jobs/{job_id}":
    let client = initCloudflareClient("test-key")
    client.baseUri = "http://127.0.0.1:" & $int(startMock())
    discard waitFor client.getZonesZoneIdLogpushJobsJobId(1, "test")

  test "DELETE /zones/{zone_id}/logpush/jobs/{job_id}":
    let client = initCloudflareClient("test-key")
    client.baseUri = "http://127.0.0.1:" & $int(startMock())
    discard waitFor client.deleteZonesZoneIdLogpushJobsJobId(1, "test")
